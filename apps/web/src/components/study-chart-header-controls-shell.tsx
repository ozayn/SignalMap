 ...
 *
 * Real controls render only after mount so extensions (e.g. password managers)
 * cannot inject attributes into SSR inputs before hydration.
 */
export function StudyChartHeaderControlsShell({ children }: { children: React.ReactNode }) {
  ...
  if (!controlsMounted) {
    return (
      <div
        className="flex min-h-[28px] min-w-0 flex-wrap items-center gap-3 pt-2"
        aria-hidden
