...
def get_wdi_gdp_global_comparison_signal(
    ...
    """Return World Bank annual GDP (levels) for US, China, Japan, Germany, India, Russia, and world (WLD)."""
    if start is None:
        start = "1960-01-01"
    if end is None:
        end = datetime.now(timezone.utc).strftime("%Y-%m-%d")
...
@app.get("/api/signals/wdi/iran-money-supply-m2")
def get_wdi_iran_money_supply_m2_signal(
    start: str | None = Query(None, description="Start date YYYY-MM-DD"),
    end: str | None = Query(None, description="End date YYYY-MM-DD"),
):
    """Return Iran M2 growth: WDI FM.LBL.BMNY.ZG through 2016, then YoY from CBI-liquidity levels (static JSON). Plus CPI (FP.CPI.TOTL.ZG) for comparison."""
    if start is None:
        start = "1960-01-01"
    if end is None:
        end = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    ...
    if start > end:
        raise HTTPException(status_code=400, detail="start must be <= end")
    try:
        from signalmap.services.signals import get_iran_money_supply_m2
...
def get_wdi_iran_demand_nominal_usd_signal(
    ...
    """Iran: WDI demand aggregates — nominal (current US$) and real (constant 2015 US$)."""
    if start is None:
        start = "1960-01-01"
    if end is None:
        end = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    ...
    if start > end:
        raise HTTPException(status_code=400, detail="start must be <= end")
    try:
        from signalmap.services.signals import get_iran_demand_nominal_usd

        return get_iran_demand_nominal_usd(start, end)
    except Exception as e:
        log.exception("iran_demand_nominal_usd failed start=%s end=%s", start, end)
        raise HTTPException(status_code=502, detail=f"Signal fetch failed: {e}")
